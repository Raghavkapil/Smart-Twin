...
// Snapshot shared between sensor loop (core 1) and web server (core 0)
struct SensorSnapshot {
    ...
    float vibration;
    char  vib_level[10];
} snapshot = {0, 0.0, 0.0, 0.0, 0.0, "NONE"};
...
// ─── Read sensors, send over Serial, update WiFi snapshot ──
void sendData(int speedPercent)
{
    ...
    Serial.print(vib, 4); Serial.print(",");
    Serial.println(level);
    Serial.flush();

    // Update shared snapshot for WiFi
    if (xSemaphoreTake(snapshotMutex, pdMS_TO_TICKS(100)) == pdTRUE)
    {
        snapshot.speed_percent = speedPercent;
        snapshot.current       = current;
        snapshot.temperature   = isnan(temperature) ? 0.0f : temperature;
        snapshot.humidity      = isnan(humidity)    ? 0.0f : humidity;
        snapshot.vibration     = vib;
        strncpy(snapshot.vib_level, level.c_str(), sizeof(snapshot.vib_level) - 1);
...
void handleData()
{
    ...
    String json = "{";
    json += "\"speed_percent\":"  + String(s.speed_percent)   + ",";
    json += "\"current\":"        + String(s.current, 2)      + ",";
    json += "\"temperature\":"    + String(s.temperature, 1)  + ",";
    json += "\"humidity\":"       + String(s.humidity, 1)     + ",";
    json += "\"vibration\":"      + String(s.vibration, 4)    + ",";
    json += "\"vib_level\":\""    + String(s.vib_level)       + "\"";
    json += "}";
...
{
    ledcWrite(LPWM, pwm);
    ledcWrite(RPWM, 0);   // keep opposite direction off
    delay(500);
}
...
void setup()
{
    ...
    writeMPU(0x1A, 0x03);             // low-pass filter 44 Hz
    ...
    Serial.println(">> Paste this IP as ESP32_IP in serial_reader.py <<\n");

    // Web server on core 0 — sensor/motor loop stays on core 1
