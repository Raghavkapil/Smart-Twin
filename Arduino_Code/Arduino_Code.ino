...
void calibrate() {
  ...
  accel_x_offset = (sumAX/samples)/4096.0;
  accel_y_offset = (sumAY/samples)/4096.0;
  gyro_x_offset  = (sumGX/samples)/16.4;
  gyro_y_offset  = (sumGY/samples)/16.4;
  gyro_z_offset  = (sumGZ/samples)/16.4;
...
float readVibration(String &level_out) {
  ...
  if (Wire.available() < 14) { level_out = "ERROR"; return -1; }
  ...
  if      (filtered < 0.05) level_out = "NONE";
...
void sendData(int speedPercent) {
  ...
  if (vib < 0) Serial.print("0");
  else         Serial.print(vib, 4);
  Serial.print(",");
...
void setup() {
  ...
  writeMPU(0x6B, 0x80); delay(200);
  writeMPU(0x6B, 0x00); delay(100);
  writeMPU(0x1B, 0x18); // ±2000 dps
  writeMPU(0x1C, 0x10); // ±8g
  writeMPU(0x1A, 0x04); // stronger LPF
